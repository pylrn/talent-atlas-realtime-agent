...
| 03 | It works, and here is the proof | content | hero | peak | 巨型数字+洞察 | 大数字（≥96px）×3 + Chart | anchor | 字数约150 / 图1 / 留白约36% | 禁止等宽卡片横排；禁止把核心数字塞进图表卡的角落；禁止 L3 角标顶替 L1 | 654 tests, 12 of 12 scenarios, 2.6 ms against a 250 ms budget — and the whole graded surface runs with no database, no network and no API key. The number is only half the page; the judgement is that reproducibility was designed in, not asserted. |
